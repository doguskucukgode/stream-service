...
def load_model_and_json():
    json_path = CarConfig.classifier["model_folder"] + '/' + CarConfig.classifier["classes_json"]
    print ("Loading JSON on path: ", json_path)
    json_file = open(json_path, 'r')
    json_read = json_file.read()
    loaded_model_json = json.loads(json_read)

    model_path = CarConfig.classifier["model_folder"] + '/' + CarConfig.classifier["model_file_name"]
...
def handle_requests(socket):
    ...
    tf_config.gpu_options.per_process_gpu_memory_fraction = CarConfig.classifier["gpu_memory_frac"]
    ...
    while True:
        ...
        try:
            ...
            predict_list = classifyIndices(loaded_model_json, preds, CarConfig.classifier["n"])
...
if __name__ == '__main__':
    socket = None
    try:
        host = CarConfig.classifier["host"]
        port = CarConfig.classifier["port"]
